...
| Generating a document from a PCO whose review is unfinished | Nothing stops it, and mostly that is right — an unsigned change order is what you send out to get signed, and the row's status and review chain are left alone, so it moves no money. Two things are wrong. The PDF says CHANGE ORDER No. PCO-GC-001 whatever the button said, which overstates a proposal that nobody has approved. And proposals ticked to be rolled in are closed out against it — marked "Rolled into PCO-GC-001" and their own reviews stopped — so approved work gets absorbed into something not yet approved, and cannot be covered by anything else until that PCO becomes a change order. Needs a decision from you on whether rolling should require the covering row to be approved first |
