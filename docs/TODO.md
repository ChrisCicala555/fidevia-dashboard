...
| Payment applications have workflow columns but no workflow UI | The columns are written and unused; either wire them up or drop them, and that is a product decision. Narrower now: the pencil copy review has its four outcomes and its own dialog, so what is left is the step chain itself — Fidevia review then architect review, with the chase and reminder behaviour the other modules have |
