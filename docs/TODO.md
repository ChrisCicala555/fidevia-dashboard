...
| Notifications set per company, not per person | Christopher, 17 Sep. Every notify column is a tick against one person, so a firm's subscription is really a handful of separate ones that have to agree. A new engineer joins Engineer 1 and is silently off the thread until somebody notices and ticks their row; somebody leaves and their tick goes on meaning nothing. It is also why the payment application audience took four goes to get right — each attempt was trying to infer, from something about a person, a fact that belongs to their firm. The shape: the toggles move to the contractor/firm list, and a contact inherits their company's answer, with a per-person override for the cases that need one (a PM who wants RFIs and not payment applications). Wants a decision on what happens to the ticks already set — promote them to the firm where a firm's people agree, and keep them as overrides where they do not |
| An architect or engineer cannot record a payment application review | Christopher, 17 Sep: "I am the architect — how can I review this?" They cannot. The Action column offers Take Action to Fidevia alone, so a design reviewer sitting on a pencil copy step has no way to answer it, and the chain stops there until Fidevia signs on their behalf as an override. Every other module has this through the reply dialog; payment applications never got one. Four things are needed and none is hard on its own: the button shown to whoever the current step belongs to, not only to Fidevia; the dialog writing through updateRow rather than the whole-file uploadText, which is Fidevia's alone and is the 403 this would otherwise hit; the server's payRowReviewed lock relaxed so that a reviewer the chain actually names is not turned away by the mark Fidevia's own review left; and Reviewed By, Review Date and Action added to the fields a reviewer may write, or the outcome kept in the workflow columns alone. Worth doing next — it is the missing half of the pencil copy workflow |
| An admin panel for unmatched access requests | Requests that match no project currently go nowhere visible |
| A cross-project view of what the dashboard has sent | The Notification Log in a project's settings used to offer "All projects", which put one client's emails on another client's settings tab. Removed rather than kept, because the panel is inside a project and everything beside it is about that project. "Has anything sent at all today" is a real question, but it is an administrator's, and it belongs wherever the other cross-project tools end up — alongside the unmatched access requests above. The server already takes a blank projectId and returns everything, so the work is a home for it, not a query |
