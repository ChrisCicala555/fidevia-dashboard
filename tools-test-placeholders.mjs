...
// Examples should be obviously invented and obviously round — and labelled as
// examples, because "Panelboard shop drawings" sitting in a submittal field is
// a plausible enough description to be read as this job's data rather than as a
...
// The prefix is the label. Stripped before the shape of the example is checked,
// so the two rules stay separate: is it plainly an example, and is the example
// itself round.
const eg=v=>String(v==null?'':v).replace(/^e\.g\.\s*/,'');
...
MONEY.forEach(id=>{
  ...
  if(p===null) return;
  ok(/^e\.g\. /.test(p), id+' is labelled as an example — got '+JSON.stringify(p));
  ok(/^(0|[1-9][0-9]?[0-9]?(,[0-9]{3})*)$/.test(eg(p)), id+' is a plain whole number — got '+JSON.stringify(p));
  const digits=eg(p).replace(/,/g,'');
  ok(digits==='0' || /^[1-9]0*$/.test(digits) || /^[1-9][05]0*$/.test(digits),
     id+' is round enough to be obviously made up — got '+p);
});
{
  const big=MONEY.map(of).filter(Boolean).map(p=>eg(p).replace(/,/g,'')).filter(d=>d.length>3);
  ok(big.every(d=>d.length>3), 'sanity: there are four-figure examples to check');
  ok(MONEY.map(of).filter(Boolean).map(eg).every(p=>p==='0' || p.replace(/,/g,'').length<4 || p.includes(',')),
...
ok(eg(of('prof-first'))==='Jane' && eg(of('prof-last'))==='Doe', 'the profile form uses one invented person');
ok(eg(of('cdn-first'))==='Jane' && eg(of('cdn-last'))==='Doe', 'and the directory form uses the same one');
ok(eg(of('cdn-email'))==='jane.doe@example.com',
...
all.filter(p=>/^(e\.g\. )?\(\d{3}\)/.test(p)).forEach(p=>{
  ok(/^\(555\) 555-01\d\d$/.test(eg(p)),
...
ok(eg(of('og-line1'))==='123 Main Street', 'which is the street everyone already reads as a placeholder');
ok(eg(of('np-zip'))==='12345', 'and a ZIP that is a counting sequence, not a town');
...
ok(all.includes('e.g. 03-300'), 'the cost code still shows a CSI division');
// The prefix is the label, so every example carries it and nothing that is an
// instruction does. A field that says "Describe the work completed" is telling
...
// nonsense.
{
  ...
  ok(wrong.length===0, 'and no instruction was given one — '+JSON.stringify(wrong));
  ok(all.filter(p=>/^e\.g\./.test(p)).length>=45,
     'across the whole dashboard rather than the handful that had it before');
}
